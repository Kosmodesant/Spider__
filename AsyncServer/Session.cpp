...
void Session::createResponsePost()
{
    if (m_request.target() == "/")
    {
        ...
        // Преобразование поискового запроса в нижний регистр
        ...
        while (true)
        {
            ...
            pos = s.find('+');
            if (pos != std::string::npos) {
                // Преобразование каждого слова в нижний регистр
                std::string wordToAdd = s.substr(0, pos);
                std::transform(wordToAdd.begin(), wordToAdd.end(), wordToAdd.begin(), ::tolower);
                word.push_back(wordToAdd);
            }
            else {
                if (!s.empty()) {
                    // Преобразование последнего слова в нижний регистр
                    std::transform(s.begin(), s.end(), s.begin(), ::tolower);
                    word.push_back(s);
                }
                break;
        ...
        if (word.empty() == false) {
            ...
            {
                Clientdb db(m_cdata);
                // Выполнение поиска по словам в нижнем регистре
                LinkAmount = db.getLinkAmount(word);
            }
            catch (const pqxx::broken_connection& err)
            {
