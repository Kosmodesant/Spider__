...
//
//        // наполнение кэша ссылками из БД
...
//
//        // проверка на наличие ссылки в кэше БД
...
static std::string removeFragmentFromUrl(const std::string& url) {
    size_t fragmentPos = url.find('#');
    if (fragmentPos != std::string::npos) {
        return url.substr(0, fragmentPos);
    }
    else {
        return url; // URL без фрагмента
    }
}
    ...
    Thread_pool& threadPool, std::shared_ptr<Clientdb> db_ptr)
{
    ...
    try
    {
        ...
        // проверка на наличие ссылки в кэше БД
        std::string cleanUrl = removeFragmentFromUrl(url_str);
        if (!cachelink.addLink(cleanUrl)) {
            // URL уже есть в базе, пропускаем
            ul_console.lock();
            consoleCol(col::blue);
            std::wcout << L"Уже есть, пропускаю: " << utf82wideUtf(cleanUrl) << std::endl;
            consoleCol(col::cancel);
            ul_console.unlock();
            return;
        }
